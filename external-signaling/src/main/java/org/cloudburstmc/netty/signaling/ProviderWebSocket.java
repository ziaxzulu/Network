                ...
                switch (name) {
                    ...
                    case "headers" -> {
                        ...
                        while (reader.hasNext()) {
                            ...
                            if (key.length() > 128 || !key.matches("[!#$%&'*+.^_`|~0-9A-Za-z-]+")
                                    || headers.size() >= 32) throw new IOException("Invalid response header");
                            key = key.toLowerCase(Locale.ROOT);
                            if (headers.containsKey(key)) throw new IOException("Duplicate response header");
