...
class ProviderDiagnosticsTest {
    ...
    static final class Transport extends ProviderClientTest.FakeTransport {
        ...
        @Override public CompletionStage<HostProfileSnapshot> captureHostProfile() {
            ...
            return hostProfile().thenApply(profile -> {
                ...
                if (withdrawn) profile.add("candidates", new JsonArray());
    ...
    static final class Fixture implements AutoCloseable {
        ...
        ProviderClient client(Path directory, Transport transport, boolean enabled, String method) throws Exception {
            ...
            return new ProviderClient(config, new ProviderStateStore(directory), transport, () -> null,
                    () -> new ProviderClient.Health(true, 10, "fixture", null), ignored -> { });
        }
        public void close() {
    ...
    @Test @Timeout(30) void withdrawnCandidatesRemainPublishedForRecoveryAndDisappearAfterRestoration() throws Exception {
        try (var f = new Fixture()) {
            var transport = new Transport();
            var client = f.client(directory, transport, true, "discovered");
            try {
                ...
                assertEquals(1, transport.policies.get(transport.policies.size() - 1).endpoints().size());
                transport.withdrawn = false;
                transport.publicationVersion++;
                client.readiness().get(10, TimeUnit.SECONDS);
                data = f.provider.lastHeartbeat.getAsJsonObject("extensions").getAsJsonObject(NAMESPACE).getAsJsonObject("data");
                assertFalse(data.has("probeCandidates"), "Matching lists need no duplicate publication");
            } finally { client.stop().toCompletableFuture().get(10, TimeUnit.SECONDS); }
        }
    }
