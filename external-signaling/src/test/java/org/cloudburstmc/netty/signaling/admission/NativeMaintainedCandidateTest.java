...
class NativeMaintainedCandidateTest {
    ...
    void withdrawnEndpointRetainsDiagnosticAuthorityForRecovery() throws Exception {
        try (var host = new Host(Map.of(), List.of("8.8.8.8"))) {
            ...
            var context = new DiagnosticAdmissionCodec.Context(
                    "https://provider.example", "recovery-host",
                    before.profile().getAsJsonObject("statelessAdmission").get("incarnation").getAsString(), 1);
            var endpoint = new DiagnosticHostPolicy.Endpoint(4,
                    DiagnosticAdmissionCodec.address(4, "8.8.8.8"), host.port, before.candidateRevision());
            var policy = new DiagnosticHostPolicy(context,
                    List.of(new DiagnosticAdmissionCodec.Key("K001", TestSignalingProvider.SECRET, 0, expires)),
                    Set.of(endpoint), expires);
            host.transport.configureDiagnostics(policy).toCompletableFuture().get(5, TimeUnit.SECONDS);
            long now = System.currentTimeMillis();
            var target = new InetSocketAddress("8.8.8.8", host.port);
            host.transport.reportConnectivityChecks(before.candidateRevision(), List.of(new ProviderTransport.ConnectivityCheck(
                    4, "discovered", target, ProviderTransport.ConnectivityOutcome.NOT_ESTABLISHED, now - 1, now + 30_000))).toCompletableFuture().get();
            ...
            host.transport.configureDiagnostics(policy).toCompletableFuture().get(5, TimeUnit.SECONDS);
            host.transport.reportConnectivityChecks(before.candidateRevision(), List.of(new ProviderTransport.ConnectivityCheck(
                    4, "discovered", target, ProviderTransport.ConnectivityOutcome.ESTABLISHED, System.currentTimeMillis(), now + 30_000))).toCompletableFuture().get();
            assertEquals(1, host.transport.captureHostProfile().toCompletableFuture().get().profile().getAsJsonArray("candidates").size());
