...
That preflight checks handoff structure, generated scripts, profile plans, curve matrix coverage, contention scenario coverage, and whether contention plan rows keep the handoff's receiver-total client count and per-client Mbps target. The underlying perfect-network baseline plan is equivalent to:
