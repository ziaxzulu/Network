...
| Affected cohort observed queue | at most 8 MiB, using timeline current/high-water totals rather than a per-peer summary |
...
- affected NACK retransmitted datagram and byte deltas, plus datagrams/second;
- affected timeout retransmitted datagram and byte deltas, plus
  datagrams/second; and
- affected transport bytes still in flight at T+10 above the pre-event gauge.

Bytes in flight are unacknowledged transport state, not inferred application
delivery. Server timelines correctly label useful event-window delivery as
...
reduction is reported, but acceptance uses the independent 8 MiB affected
cohort cap for the event maximum and T+10 value.
