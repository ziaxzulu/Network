...
The handoff manifest records the `benchmark/docs/production-usage-evidence.md` SHA-256 fingerprint and the `capture-production-evidence.sh` audit fingerprint so promoted lab artifacts can be traced back to the exact source revisions used to choose the matrix. The wrapper summary, `fresh-handoff-summary.json`, repeats the production-evidence hash, source-audit hash, source revisions, planned row counts, preflight result, and generated execution paths for the README, artifact checklist, prereq helper, perfect plan, impairment plan, and promotion helper. CI or lab orchestration can read that one machine-friendly file before execution instead of scraping the README.
The same summary includes `executionEnvironment.labExecutable` and advisory reasons. `ready=true` means the handoff structure and source audit passed; `labExecutable=false` means it was generated with local placeholder topology values such as `127.0.0.1` or `lo` and should be regenerated with the real server host and lab NIC before a separate-host baseline run.
...
Keep the generated `artifact-collection.md` open while copying artifacts back from lab hosts. Its sibling `artifact-collection.json` is the machine-readable contract for required topology, host capture, prereq, worker, netem, summary, promotion, and readiness artifacts.
...
Run the generated `check-plan-freshness.sh` before starting workers. It fails when scheduled start timestamps have passed or are too close to now; set `MIN_LEAD_SECONDS` when the lab needs a larger setup buffer. The script also writes `plan-freshness.json` beside itself so the merge/control host has durable proof of the start-window check. Then run the generated host-capture script on every host, followed by the curve and contention worker scripts in the order shown in the plan README. After receiver artifacts are copied back, run `merge-all.sh`; it writes `combined/suite-aggregate.jsonl` and combined curve `bandwidth-capacity.*` selector artifacts beside it.
