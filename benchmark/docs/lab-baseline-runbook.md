...
The readiness report is the final artifact-level gate for accepting the baseline package set. It requires passing lab validation without bypass markers, separate-host evidence, required scenario families, selected capacity groups, passing impairment profile validation without bypass allowances, required netem status evidence, and the expected impairment profiles.
