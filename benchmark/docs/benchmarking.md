...
The primary benchmark shape is server-to-client bulk traffic with a separate small probe stream. Bulk messages retain the configured workload reliability at normal priority. Probe requests and ACKs use exactly `RELIABLE_ORDERED/HIGH` through the weighted scheduler and share the workload's ordering and recovery behavior, so probe RTT includes loss recovery and head-of-line delay. The server tracks globally unique probe IDs in one active measurement window; warmup, duplicate, wrong-peer, and late prior-window ACKs cannot enter the RTT histogram.
