...

{
  ...
  if [[ -n "$handoff_root" ]]; then
    ...
    echo "- Fresh handoff lab executable: \`$(jq -r 'if .handoff.summary.executionEnvironment == null then "unknown" elif .handoff.summary.executionEnvironment.labExecutable then "true" else "false" end' "$readiness_json")\`"
    echo "- Fresh handoff topology advisory: \`$(jq -r '.handoff.summary.executionEnvironment.advisory // "missing"' "$readiness_json")\`"
    echo "- Fresh handoff topology advisory reasons: \`$(jq -r '(.handoff.summary.executionEnvironment.advisoryReasons // []) | join(",")' "$readiness_json")\`"
    echo "- Fresh handoff README: \`$(jq -r '.handoff.summary.execution.readme // "missing"' "$readiness_json")\`"
    echo "- Fresh handoff artifact collection JSON: \`$(jq -r '.handoff.summary.execution.artifactCollectionJson // "missing"' "$readiness_json")\`"
    echo "- Fresh handoff artifact collection checklist: \`$(jq -r '.handoff.summary.execution.artifactCollectionMd // "missing"' "$readiness_json")\`"
    echo "- Fresh handoff perfect-network plan: \`$(jq -r '.handoff.summary.execution.perfectPlan // "missing"' "$readiness_json")\`"
    echo "- Fresh handoff impairment plan: \`$(jq -r '.handoff.summary.execution.impairmentPlan // "missing"' "$readiness_json")\`"
    echo "- Fresh handoff promotion helper: \`$(jq -r '.handoff.summary.execution.promoteScript // "missing"' "$readiness_json")\`"
    echo "- Fresh perfect artifact root: \`$(jq -r '.handoff.summary.execution.perfectArtifacts // "missing"' "$readiness_json")\`"
    echo "- Fresh impairment artifact root: \`$(jq -r '.handoff.summary.execution.impairmentArtifacts // "missing"' "$readiness_json")\`"
  fi
