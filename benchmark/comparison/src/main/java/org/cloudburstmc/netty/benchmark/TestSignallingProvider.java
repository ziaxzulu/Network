...
final class TestSignallingProvider {
    ...
    static Answer answer(String offer, String fingerprint, int port, long expiry, String audience, boolean wrongClientFingerprint) throws Exception {
        ...
        byte[] ticketId = new byte[16]; new SecureRandom().nextBytes(ticketId);
        claims.putInt((int)(expiry / 1000)).put(fp).putShort((short)5000).putInt(262144).put(ticketId).putLong(42).put((byte)pwd.length()).put(utf8(pwd));
